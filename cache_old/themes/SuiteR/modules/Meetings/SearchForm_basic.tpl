...
<input type='text' name='{$fields.name_basic.name}' 
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='gi_products_meetings_1_name_basic' >{sugar_translate label='LBL_GI_PRODUCTS_MEETINGS_1_FROM_GI_PRODUCTS_TITLE' module='Meetings'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
<input type="text" name="{$fields.gi_products_meetings_1_name_basic.name}"  class="sqsEnabled"   id="{$fields.gi_products_meetings_1_name_basic.name}" size="" value="{$fields.gi_products_meetings_1_name_basic.value}" title='' autocomplete="off"  >
<input type="hidden"  id="{$fields.gi_products_meetings_1gi_products_ida_basic.name}" value="{$fields.gi_products_meetings_1gi_products_ida_basic.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.gi_products_meetings_1_name_basic.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.gi_products_meetings_1_name_basic.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"gi_products_meetings_1gi_products_ida_basic","name":"gi_products_meetings_1_name_basic"}}{/literal}, "single", true);'>{sugar_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button><button type="button" name="btn_clr_{$fields.gi_products_meetings_1_name_basic.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.gi_products_meetings_1_name_basic.name}.value = ''; this.form.{$fields.gi_products_meetings_1gi_products_ida_basic.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
</span>
...
value="1" title='' tabindex="" {$checked} >
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='date_start_basic' >{sugar_translate label='LBL_DATE' module='Meetings'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
{assign var="id" value=$fields.date_start_basic.name }

{if isset($smarty.request.date_start_basic_range_choice)}
{assign var="starting_choice" value=$smarty.request.date_start_basic_range_choice}
{else}
...
{html_options options=$fields.date_start_basic.options selected=$starting_choice}
</select>
</div>

<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.range_date_start_basic)  || $starting_choice == 'between'}display:none{else}display:''{/if};">
<input autocomplete="off" type="text" name="range_{$id}" id="range_{$id}" value='{if empty($smarty.request.range_date_start_basic) && !empty($smarty.request.date_start_basic)}{$smarty.request.date_start_basic}{else}{$smarty.request.range_date_start_basic}{/if}' title=''   size="11" class="dateRangeInput">
...
{assign var=date_value value=$fields.date_start_basic.value }
<input autocomplete="off" type="text" name="start_range_{$id}" id="start_range_{$id}" value='{$smarty.request.start_range_date_start_basic }' title=''  tabindex='' size="11" class="rangeDateInput">
...
{$APP.LBL_AND}
{assign var=date_value value=$fields.date_start_basic.value }
<input autocomplete="off" type="text" name="end_range_{$id}" id="end_range_{$id}" value='{$smarty.request.end_range_date_start_basic }' title=''  tabindex='' size="11" class="dateRangeInput" maxlength="10">
