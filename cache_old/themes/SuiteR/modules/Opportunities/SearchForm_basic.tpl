...
value="1" title='' tabindex="" {$checked} >
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='account_name_basic' >{sugar_translate label='LBL_ACCOUNT_NAME' module='Opportunities'}</label>
    	</td>
...
<button type="button" name="btn_{$fields.account_name_basic.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.account_name_basic.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"account_id_basic","name":"account_name_basic"}}{/literal}, "single", true);'>{sugar_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button><button type="button" name="btn_clr_{$fields.account_name_basic.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.account_name_basic.name}.value = ''; this.form.{$fields.account_id_basic.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='sales_stage_basic' >{sugar_translate label='LBL_SALES_STAGE' module='Opportunities'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
{html_options id='sales_stage_basic' name='sales_stage_basic[]' options=$fields.sales_stage_basic.options size="6" class="templateGroupChooser" multiple="1" selected=$fields.sales_stage_basic.value}
   	   	</td>
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='status_c_basic' >{sugar_translate label='LBL_STATUS' module='Opportunities'}</label>
    	</td>
...
{html_options id='status_c_basic' name='status_c_basic[]' options=$fields.status_c_basic.options size="6" class="templateGroupChooser" multiple="1" selected=$fields.status_c_basic.value}
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='customer_lpo_c_basic' >{sugar_translate label='LBL_CUSTOMER_LPO' module='Opportunities'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
{if strlen($fields.customer_lpo_c_basic.value) <= 0}
{assign var="value" value=$fields.customer_lpo_c_basic.default_value }
{else}
{assign var="value" value=$fields.customer_lpo_c_basic.value }
{/if}  
<input type='text' name='{$fields.customer_lpo_c_basic.name}' 
    id='{$fields.customer_lpo_c_basic.name}' size='30' 
    maxlength='255' 
    value='{$value}' title=''      >
   	   	</td>
