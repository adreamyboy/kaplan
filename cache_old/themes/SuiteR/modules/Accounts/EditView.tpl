...
{if $bean->field_defs.tier_c.acl < 1}
<td valign="top" id='tier_c_label' width='12.5%' scope="row">
{capture name="label" assign="label"}{sugar_translate label='LBL_TIER' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
<td valign="top" width='37.5%' >
...
<select name="{$fields.tier_c.name}" 
id="{$fields.tier_c.name}" 
title='Tier 1: xxxx, Tier 2: xxxx, Tier 3: xxxx'       
>
{if isset($fields.tier_c.value) && $fields.tier_c.value != ''}
{html_options options=$fields.tier_c.options selected=$fields.tier_c.value}
{else}
{html_options options=$fields.tier_c.options selected=$fields.tier_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.tier_c.options }
{capture name="field_val"}{$fields.tier_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.tier_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.tier_c.name}" 
id="{$fields.tier_c.name}" 
title='Tier 1: xxxx, Tier 2: xxxx, Tier 3: xxxx'          
>
{if isset($fields.tier_c.value) && $fields.tier_c.value != ''}
...
<input
id="{$fields.tier_c.name}-input"
name="{$fields.tier_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.tier_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.tier_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.tier_c.name}-input', '{$fields.tier_c.name}');sync_{$fields.tier_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.tier_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.tier_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.tier_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.tier_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.tier_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.tier_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.tier_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{if $bean->field_defs.phone_office.acl < 1}
...
{if $bean->field_defs.industry.acl < 1}
...
{if $bean->field_defs.phone_alternate.acl < 1}
...
{if $bean->field_defs.employees.acl < 1}
...
{if $bean->field_defs.contacts_accounts_1_name.acl < 1}
<td valign="top" id='contacts_accounts_1_name_label' width='12.5%' scope="row">
{capture name="label" assign="label"}{sugar_translate label='LBL_CONTACTS_ACCOUNTS_1_FROM_CONTACTS_TITLE' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
<td valign="top" width='37.5%' >
...
<input type="text" name="{$fields.contacts_accounts_1_name.name}" class="sqsEnabled" tabindex="0" id="{$fields.contacts_accounts_1_name.name}" size="" value="{$fields.contacts_accounts_1_name.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.contacts_accounts_1_name.id_name}" 
id="{$fields.contacts_accounts_1_name.id_name}" 
value="{$fields.contacts_accounts_1contacts_ida.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.contacts_accounts_1_name.name}" id="btn_{$fields.contacts_accounts_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_LABEL"}"
onclick='open_popup(
"{$fields.contacts_accounts_1_name.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"contacts_accounts_1contacts_ida","name":"contacts_accounts_1_name"}}{/literal}, 
"single", 
true
);' ><img src="{sugar_getimagepath file="id-ff-select.png"}"></button><button type="button" name="btn_clr_{$fields.contacts_accounts_1_name.name}" id="btn_clr_{$fields.contacts_accounts_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.contacts_accounts_1_name.name}', '{$fields.contacts_accounts_1_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_LABEL"}" ><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.contacts_accounts_1_name.name}']) != 'undefined'",
		enableQS
);
</script>
...
{if $bean->field_defs.website.acl < 1}
...
{if $bean->field_defs.individual_c.acl < 1}
<td valign="top" id='individual_c_label' width='12.5%' scope="row">
{capture name="label" assign="label"}{sugar_translate label='LBL_INDIVIDUAL' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
<td valign="top" width='37.5%' >
...
{if strval($fields.individual_c.value) == "1" || strval($fields.individual_c.value) == "yes" || strval($fields.individual_c.value) == "on"} 
{assign var="checked" value='checked="checked"'}
{else}
{assign var="checked" value=""}
{/if}
<input type="hidden" name="{$fields.individual_c.name}" value="0"> 
<input type="checkbox" id="{$fields.individual_c.name}" 
name="{$fields.individual_c.name}" 
...
<tr>
<th align="left" colspan="8">
<h4>{sugar_translate label='LBL_EDITVIEW_PANEL1' module='Accounts'}</h4>
</th>
</tr>
{counter name="fieldsUsed" start=0 print=false assign="fieldsUsed"}
{capture name="tr" assign="tableRow"}
...
{if $bean->field_defs.billing_address_country.acl < 1}
...
{if $bean->field_defs.billing_address_city.acl < 1}
...
{if $bean->field_defs.billing_address_postalcode.acl < 1}
...
addToValidate('EditView', 'campaign_name', 'relate', true,'{/literal}{sugar_translate label='LBL_CAMPAIGN' module='Accounts' for_js=true}{literal}' );
...
addToValidate('EditView', 'first_invoice_date', 'kreporter', false,'{/literal}{sugar_translate label='LBL_FIRST_INVOICE_DATE' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'individual_c', 'bool', false,'{/literal}{sugar_translate label='LBL_INDIVIDUAL' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'tier_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TIER' module='Accounts' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'campaign_name', 'alpha', true,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Accounts' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CAMPAIGN' module='Accounts' for_js=true}{literal}', 'campaign_id' );
addToValidateBinaryDependency('EditView', 'contacts_accounts_1_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Accounts' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CONTACTS_ACCOUNTS_1_FROM_CONTACTS_TITLE' module='Accounts' for_js=true}{literal}', 'contacts_accounts_1contacts_ida' );
