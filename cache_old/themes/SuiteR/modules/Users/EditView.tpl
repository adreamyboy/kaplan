...
<h4>{sugar_translate label='LBL_USER_INFORMATION' module='Users'}</h4>
</th>
</tr>
...
{if $bean->field_defs.user_name.acl < 1}
...
{if $bean->field_defs.UserType.acl < 1}
...
{if $bean->field_defs.picture.acl < 1}
...
<tr>
<th align="left" colspan="8">
<h4>{sugar_translate label='LBL_EMPLOYEE_INFORMATION' module='Users'}</h4>
</th>
</tr>
{counter name="fieldsUsed" start=0 print=false assign="fieldsUsed"}
{capture name="tr" assign="tableRow"}
...
{if $bean->field_defs.employee_status.acl < 1}
...
{if $bean->field_defs.show_on_employees.acl < 1}
...
{if $bean->field_defs.reports_to_name.acl < 1}
...
{if $bean->field_defs.phone_other.acl < 1}
...
{if $bean->field_defs.messenger_type.acl < 1}
...
{if $bean->field_defs.messenger_id.acl < 1}
...
{if $bean->field_defs.phone_home.acl < 1}
...
{if $bean->field_defs.address_street.acl < 1}
...
{if $bean->field_defs.address_city.acl < 1}
...
{if $bean->field_defs.address_state.acl < 1}
...
{if $bean->field_defs.address_postalcode.acl < 1}
...
{if $bean->field_defs.address_country.acl < 1}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_NOTES' module='Users'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
<td valign="top" width='37.5%' >
{counter name="panelFieldCount"}
<textarea tabindex="0"  id="description" name="description">{$fields.description.value}</textarea>{literal}<script type="text/javascript" language="Javascript" src="include/javascript/tiny_mce/tiny_mce.js?v=xnWEBJc8xvNdS4iSEgE4ng"></script>
...
{if $bean->field_defs.photo.acl < 1}
...
{if $fields.photo.value|strstr:"http"}
<a href="{$fields.photo.value}"</a>
{else}
