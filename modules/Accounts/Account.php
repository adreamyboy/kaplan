...
class Account extends Company {
	...
	*/
    ...
    /************
     * Custom functions for Genesis
     */
    function get_individual_contact() {
        if ($this->individual_c) {
            $link = 'contacts';
    ...
    function can_be_deleted() {
        ...
        foreach ($linked_fields as $link => $value) {
            if (!in_array($link, $ignored_rel)) {
                if ($this->load_relationship($link)) {
                    $count = count($this->$link->getBeans());
                    if ($count > 0 && $link != 'contacts') {
                        return false;
                    } else {
                        if (!$this->individual_c || $count != 1) {
                            return false;
                        }
                    }
                }
