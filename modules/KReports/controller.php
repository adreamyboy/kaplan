...
class KReportsController extends SugarController {
   ...
   function action_get_userids() {
      ...
      $usersResult = $db->query('SELECT id, user_name, first_name, last_name FROM users WHERE deleted = \'0\' AND status = \'Active\' AND user_name like \'' . $_REQUEST['query'] . '%\' LIMIT ' . $_REQUEST['start'] . ',' . $_REQUEST['limit']);
      //else
   ...
   function action_get_nodes() {
      ...
      if ($_REQUEST['node'] != 'unionroot') {
         ...
         if ($nodeArray[0] == 'relate') {
            ...
            $returnJArray = json_encode_kinamu($this->buildNodeArray($nodeModule->field_defs[$nodeArray[2]]['module'], 'TREE', $nodeModule->$nodeArray['2'], $nodeArray['2']));
      ...
      if ($module != '' && $module != 'undefined' && file_exists($beanFiles[$beanList[$module]])) {
         ...
         foreach ($nodeModule->field_name_map as $field_name => $field_defs) {
            if ($field_defs['type'] != 'link'
                    && (!isset($field_defs['reportable']) || (isset($field_defs['reportable']) && $field_defs['reportable'] == true))
