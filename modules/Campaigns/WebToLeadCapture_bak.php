...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
	    ...
	    if(isset($camp_data) && $camp_data != null ){
            ...
            /* else if (exists($lead->phone_mobile)){
                    $lead->save(FALSE);
                }*/
                
               /* global $db;
                
                $dupesql_mngr = "Select * from leads where first_name= '".$_POST['first_name']."' and phone_mobile= '".$_POST['phone_mobile']."'and last_name =  '".$_POST['last_name']."' and deleted=0 ";
               // print_r($dupesql_mngr);
		        $duperaw_mngr = $db->query($dupesql_mngr);
		     			
                
			if($duperaw_mngr->num_rows >1){
				 while($row_mngr = $db->fetchByAssoc($duperaw_mngr)){
					$dupleadid = $row_mngr['id'];
				$usrUp = "Update leads SET deleted = '1'  WHERE id = '".$dupleadid."' ";  
				//print_r($usrUp);	
		        $RObj  = $db->query($usrUp);
			}
		}//die;*/
            ...
            //global $db;
                ...
                $dupesql_mngr = "Select * from leads where first_name= '".$_POST['first_name']."' and phone_mobile= '".$_POST['phone_mobile']."'and last_name =  '".$_POST['last_name']."' and description =  '".$_POST['description']."' and DATE(date_entered) = date('Y-m-d')  and deleted=0 ";
               // print_r($dupesql_mngr);
		        $duperaw_mngr = $db->query($dupesql_mngr);
           ...
           // $lead = $leadForm->handleSave($prefix, false, false, false, $lead);
			...
			if(!empty($lead)){
                ...
                $dupesql_mngr = "Select 1 from leads where first_name= '".$_POST['first_name']."' and phone_mobile= '".$_POST['phone_mobile']."'and last_name =  '".$_POST['last_name']."' and description =  '".$_POST['description']."' and DATE(date_entered) = date('Y-m-d')  and deleted=0 ";
               // print_r($dupesql_mngr);
		        $duperaw_mngr = $db->query($dupesql_mngr);
