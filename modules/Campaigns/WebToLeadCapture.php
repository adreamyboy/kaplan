...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
	    ...
	    if(isset($camp_data) && $camp_data != null ){
			...
			if(!empty($lead)){
				$leadID    = $lead->id;
				$leadPhone = $lead->phone_mobile;
				$leadFname = $lead->first_name;
				$leadLname = $lead->last_name;
				
				global $db;
				$sql = "delete FROM `leads` where phone_mobile = '$leadPhone' and first_name = '$leadFname' and last_name = '$leadLname' and id!='$leadID' ";
				$db->query($sql);
	            //create campaign log
