...
//Code for autopopulate from name field 
 ?>
    <input type="hidden" id="phpVar" value="<?php echo $campId; ?>">
     <script type='text/javascript'>
		 $(document).ready(function(){
			 
             $('#inbound_email_id').change(function() {
				 var reply_name = $('#reply_name').val();
				 var reply_addr = $('#reply_addr').val();
				  $('#from_name').val(reply_name);
				  $('#from_addr').val(reply_addr);
				var Id =  $('#phpVar').val();
				...
				$.ajax({
								...
								url: 'index.php?entryPoint=Set_FromName',
								method: 'POST',
								data: { reply_name : reply_name, reply_addr : reply_addr, Id : Id},
