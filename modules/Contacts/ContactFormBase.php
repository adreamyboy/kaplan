...
public function getDuplicateQuery($focus, $prefix='')
{
    ...
    $query='SELECT c.id, c.first_name, c.last_name, c.title, c.phone_mobile, c.phone_work, c.phone_home, c.phone_other, e.email_address FROM contacts c
            LEFT OUTER JOIN email_addr_bean_rel b ON (c.id = b.bean_id AND (b.bean_module = "Contacts" AND b.primary_address = 1))
            LEFT OUTER JOIN email_addresses e ON (b.email_address_id = e.id AND e.deleted =0)
            ';

    $query .= ' WHERE c.deleted = 0 ';

    if(!empty($_POST[$prefix.'record'])) {
        $query .= " AND c.id != '". $_POST[$prefix.'record'] ."'";
    }

    $filters = array();

    if(isset($_POST[$prefix.'first_name']) && strlen($_POST[$prefix.'first_name']) != 0 && isset($_POST[$prefix.'last_name']) && strlen($_POST[$prefix.'last_name']) != 0){
        $filters[] = "(c.first_name LIKE '%". $_POST[$prefix.'first_name'] . "%' OR c.first_name SOUNDS LIKE '". $_POST[$prefix.'first_name'] . "') AND c.last_name SOUNDS LIKE '". $_POST[$prefix.'last_name'] ."'";
    } else {
        $filters[] = "c.last_name SOUNDS LIKE '". $_POST[$prefix.'last_name'] ."'";
    }

    $phone_work = $_POST[$prefix.'phone_work'];
    $phone_work = str_replace(' ', '', $phone_work);
    $phone_work = substr($phone_work, -8);
    if(!empty($phone_work)) {
        $filters[] = "c.phone_work LIKE '%{$phone_work}' OR c.phone_mobile LIKE '%{$phone_work}' OR c.phone_home LIKE '%{$phone_work}' OR c.phone_other LIKE '%{$phone_work}'";
    }

    $phone_mobile = $_POST[$prefix.'phone_mobile'];
    $phone_mobile = str_replace(' ', '', $phone_mobile);
    $phone_mobile = substr($phone_mobile, -8);
    if(!empty($phone_mobile)) {
        $filters[] = "c.phone_work LIKE '%{$phone_mobile}' OR c.phone_mobile LIKE '%{$phone_mobile}' OR c.phone_home LIKE '%{$phone_mobile}' OR c.phone_other LIKE '%{$phone_mobile}'";
    }

    $phone_home = $_POST[$prefix.'phone_home'];
    $phone_home = str_replace(' ', '', $phone_home);
    $phone_home = substr($phone_home, -8);
    if(!empty($phone_home)) {
        $filters[] = "c.phone_work LIKE '%{$phone_home}' OR c.phone_mobile LIKE '%{$phone_home}' OR c.phone_home LIKE '%{$phone_home}' OR c.phone_other LIKE '%{$phone_home}'";
    }

    $phone_other = $_POST[$prefix.'phone_other'];
    $phone_other = str_replace(' ', '', $phone_other);
    $phone_other = substr($phone_other, -8);
    if(!empty($phone_other)) {
        $filters[] = "c.phone_work LIKE '%{$phone_other}' OR c.phone_mobile LIKE '%{$phone_other}' OR c.phone_home LIKE '%{$phone_other}' OR c.phone_other LIKE '%{$phone_other}')";
    }

    $email = $_POST['Leads0emailAddress0']; // BUG: This will not work if email2 is entered.  Also, it will use the email1 even if it is invalid/opt-out.
    if(!empty($email)) {
        $filters[] = "e.email_address = '{$email}'";
    }
