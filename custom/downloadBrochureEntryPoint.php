...
/*
Use:
- Check the "Access Level" to a document, and decide whether to download the file immediately, or to redirect visitor to a Web-to-Lead form (to receive the requested document via email).

Input Params:
- document_id: ID of the document to be downloaded.  This is required.
- document_name: Name of the document to be download.  This is required.
- document_type: Sub-Category value of the document.  This is optional (for backward functionality) but now it is actually ignored.

Output Params:
- Downloading the file immediately OR redirection to a special page which prompts user to fill a Web-to-Lead form ("option=com_genesisreview&view=download").
*/

// NOTE: The below code was mostly adapted from SugarCRM's default file: "download.php"
...
if((!isset($_REQUEST['isProfile']) && empty($_REQUEST['id'])) || empty($_REQUEST['type'])) {
    ...
    if(!isset($_REQUEST['isTempFile'])) {
        ...
        // Check the access permissions: Public, Protected, Internal
        
        // If document is "Protected" & (d != 1), redirect to Web-to-Lead form.
		if ($focus->access_c == 'Protected' && $_REQUEST['d'] != '1') {
			$document_id = $focus->id;
			$document_name = urlencode($focus->document_name);
			$document_type = urlencode($focus->document_subcategory_id);
			$joomla_url = $sugar_config['joomla_url'];
			header("Location:{$joomla_url}/get-doc?document_id={$document_id}&document_name={$document_name}&document_type={$document_type}");
			
		// If document is "Internal", show an die message.
        } else if ($focus->access_c == 'Internal') {
			die('This is an internal document.  Please contact us for any assistance.');
			
		}
		
		// ... Reaching to this point means that the document is either 'Public' or 'Protected' (with d=1)..
        // Thus, prepare document to get downloaded.
