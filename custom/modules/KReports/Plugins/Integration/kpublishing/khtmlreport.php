...
class khtmlreport {
    ...
    public function renderReport($reportId, $start = 0, $entries = 10, $renderTarget = 'Dashlet', $addParams = array()) {
        ...
        if ($thisReport->kQueryArray->countSelectString != '') {
            ...
            $totalCount = $db->getRowCount($queryResults = $db->query($query = $thisReport->get_report_main_sql_query(false)));
        ...
        // parse the TPL
        $reportHTML = $ss->fetch('custom/modules/KReports/Plugins/Integration/kpublishing/tpls/khtmlpublish.tpl');
