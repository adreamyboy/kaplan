...
class kexcelexport extends kreportintegrationplugin {
   ...
   public function getMenuItem() {
      return array(
          'jsFile' => 'custom/modules/KReports/Plugins/Integration/kexcelexport/kexcelexport' . ($GLOBALS['sugar_config']['KReports']['debug'] ? '_debug' : '') . '.js',
          'menuItem' => array(
              'icon' => $this->wrapText('modules/KReports/images/xls.png'),
              'text' => $this->wrapText($this->pluginName),
              'handler' => $this->wrapFunction('kreportkexcelexporthandler')
          )
      );
   }
   ...
   public function exportToExcel($reportId, $dynamicols = '', $exportto = 'xlsx') {
      ...
      $exportData = $thisPresManager->getPresentationExport($thisReport, $dynamicols, false);
      if (!$exportData)
         $exportData = $this->getDefaultPresentationExport($thisReport, $dynamicols, false);

      // some settings
      //date_default_timezone_set('Europe/London');
      date_default_timezone_set('UTC');
      define('EOL', (PHP_SAPI == 'cli') ? PHP_EOL : '<br />');
      require_once('custom/k/phpexcel/PHPExcel.php');
      ...
      switch ($exportto) {
         ...
         case 'pdf':
            ...
            $rendererLibraryPath = 'custom/k/tcpdf';
   ...
   private function getDefaultPresentationExport($thisReport, $dynamicols = '') {
      ...
      $reportResults = $thisReport->getSelectionResults(array('toCSV' => true, 'noFormat' => true));
      ...
      switch ($fieldtype) {
         case 'kcurrencyRenderer':
            ...
            if (isset($record[$fieldid . '_curid']))
               $theSheet->getActiveSheet()->getStyle($theCell)->getNumberFormat()->setFormatCode((isset($record[$fieldid . '_curid']) && isset($this->currencySymbols[$record[$fieldid . '_curid']]) ? $this->currencySymbols[$record[$fieldid . '_curid']] : '*') . ' * #,##0.00');
            else
               $theSheet->getActiveSheet()->getStyle($theCell)->getNumberFormat()->setFormatCode((isset($this->currencySymbols['-99']) ? $this->currencySymbols['-99'] : '*') . ' * #,##0.00');
            break;
         case 'kdateRenderer':
   ...
   private function loadCurrencySymbols() {
      ...
      $this->currencySymbols['-99'] = $GLOBALS['sugar_config']['default_currency_symbol'];
