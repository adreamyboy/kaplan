...
class kreportpresentationpivot extends kreportpresentationplugin {
   ...
   public function display($thisReport) {
      $viewJS .= '<script type="text/javascript" src="custom/modules/KReports/Plugins/Presentation/pivot/js/viewpivot' . ($GLOBALS['sugar_config']['KReports']['debug'] ? '_debug' : '') . '.js"></script>';
      return $viewJS;
   }

   public function getExportData($thisReport, $dynamicols = '', $renderFields = false) {
   ...
   public function generatePivot($thisReport, $thisSnapshot = 0, $thisPanelWidth = 1000, $renderFields = true) {
      ...
      $columnArr = array(
          array('text' => $thisReport->listFieldArrayById[$this->pParams['pluginData']['rowData']]['name'], 'locked' => true, 'dataIndex' => $this->pParams['pluginData']['rowData'], 'width' => $this->nameWidth),
      );

      //2013-01-22 add add Rows
      foreach ($this->pParams['pluginData']['addRowData'] as $addRowIndex => $addRowData)
         $columnArr[] = array('text' => $addRowData['name'], 'locked' => true, 'dataIndex' => $addRowData['fieldid'], 'width' => $this->nameWidth);
